...
export default function ToolDetailPage() {
  ...
  useEffect(() => {
    ...
    const org = organizationSchema(
      'HR Tools Nav',
      'https://hrtoolsnav.net',
       'Comprehensive HR management tools directory and platform for modern teams.'
    );
  ...
  if (!tool) {
    return (
      <div className="relative pt-32 px-6 text-center">
        ...
        <Link
          href="/"
          className="inline-flex items-center text-[#8B5CF6] hover:text-[#a78bfa] transition-colors"
        >
  ...
  return (
    <div className="relative pt-28 pb-20 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <Link
          href="/"
          className="inline-flex items-center text-[#c4b5fd] hover:text-[#8B5CF6] transition-colors mb-8 text-sm"
        >
          <ArrowLeft className="w-4 h-4 mr-2" /> Back to Hub
        </Link>
        ...
        <header className="mb-10">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <div className="flex items-center gap-3">
                <span className="px-3 py-1 bg-[#2a1a4e] text-[#8B5CF6] text-xs font-bold rounded-full uppercase tracking-wider border border-[#3b2566]">
                  {tool.category}
                </span>
        ...
        <div className="bg-[#1a1233] border border-[#3b2566] rounded-xl p-6 mb-10">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-bold text-[#e8e0f7] flex items-center text-base">
              <BarChart3 className="w-5 h-5 mr-2 text-[#8B5CF6]" /> SW Score
            </h2>
            <span className="text-xs text-[#a78bfa]">Powered by verified reviews & data</span>
          </div>
          <div className="space-y-3">
            <ScoreBar label="Features" score={scores.features} color="#8B5CF6" />
        ...
        <div className="bg-[#1a1233] border border-[#3b2566] rounded-xl p-6 mb-10">
          <h3 className="font-bold text-[#e8e0f7] mb-3 flex items-center text-base">
            <Users className="w-5 h-5 mr-2 text-[#8B5CF6]" /> Best For
          </h3>
        ...
        {tool.userQuotes && tool.userQuotes.length > 0 && (
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-[#e8e0f7] mb-6 flex items-center gap-2">
              <Quote className="w-6 h-6 text-[#8B5CF6]" />
              What Users Say
            </h2>
            <div className="grid gap-4">
              {tool.userQuotes.map((q, i) => (
                ...
                >
                  <Sparkles className="w-5 h-5 text-[#8B5CF6] opacity-30 absolute top-4 right-4" />
                  <p className="text-sm text-[#c4b5fd] leading-relaxed italic mb-4">
                    &ldquo;{q.quote}&rdquo;
                  </p>
                  <div className="flex items-center gap-2 text-xs text-[#a78bfa]">
                    <div className="w-8 h-8 rounded-full bg-[#2a1a4e] flex items-center justify-center text-[#8B5CF6] font-bold">
                      {q.role.charAt(0)}
                    </div>
                    <div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-lg bg-[#2a1a4e] flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                        <RtIcon className="w-5 h-5 text-[#8B5CF6]" />
                      </div>
                      <div className="min-w-0">
                        <h3 className="text-sm font-bold text-[#e8e0f7] group-hover:text-[#8B5CF6] transition-colors">
                          {rt.name}
                        </h3>
        ...
        <div className="bg-gradient-to-r from-[#2a1a4e] to-[#1a1233] border border-[#3b2566] rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <a
            ...
            className="bg-[#8B5CF6] hover:bg-[#8B5CF6] text-white px-8 py-3 rounded-full font-bold transition-all flex items-center shadow-glow-sm whitespace-nowrap"
          >
        ...
        <div className="mt-6 text-center">
          <p className="text-xs text-[#a78bfa]">
            ...
            <Link href="/disclosure" className="text-[#8B5CF6] hover:underline">
              Learn more
            </Link>
          </p>
        </div>
